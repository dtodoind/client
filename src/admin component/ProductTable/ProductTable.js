...
import { IoCloseCircle, IoAddCircle, IoRemoveCircleSharp } from 'react-icons/io5'
...
function ProductTable({inspro, ...props}) {
    ...
    const change = (e) => {
        if(e.target.value === '') {
            document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].style.display = 'inherit'
            document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].innerHTML = 'Required'
        } else {
            for(var k=0; k<Products.length; k++) {
                if(e.target.value === Products[k].ProductName) {
                    if(e.target.name === 'pname') {
                        document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].style.display = 'inherit'
                        document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].innerHTML = 'Already Exist'
                        break
                    }
                } else {
                    document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].style.display = 'none'
                    document.getElementsByClassName('edit'+e.target.name+'error'+e.target.className)[0].innerHTML = ''
                }
            }
        }
    }
            ...
            if(e.target.name === 'pdes') {
                if(pdes !== 'Required') {
                    var db_val = {
                        ...
                        Category_id: parseInt(document.getElementsByName('pcate')[0].value),
                        Image: JSON.stringify(Products[u].images),
                    ...
                    document.getElementsByClassName(Products[u].Name)[0].nextElementSibling.innerHTML = `${p[u].Category.Name}`
                    document.getElementsByClassName(Products[u].Name)[0].nextElementSibling.nextElementSibling.innerHTML = `${p[u].Description}`
                    update()

                    var lengt1 = document.getElementsByClassName('edit_icon_ind').length
    ...
    const edit = (i) => {
        ...
        for(var j=0; j<Products.length; j++) {
            if(Products[i].Name === Products[j].Name) {
                document.getElementsByClassName(Products[i].Name)[0].innerHTML = `
                    <input type="text" placeholder="Enter Product Name" id="pname" name="pname" class=${i} value='${Products[i].Name}' />
                ...
                document.getElementsByClassName(Products[i].Name)[0].nextElementSibling.nextElementSibling.innerHTML = `
                    <input type="text" style="width: 100%" placeholder="Enter description" id="pdes" name="pdes" class=${i} value="${Products[i].Description}" />
                    <div style="color: red; display: none;" class=${'editpdeserror'+i}></div>
                `
                ...
                document.getElementsByClassName(Products[i].Name)[0].nextElementSibling.innerHTML = `
                    <select id="pcate" name="pcate" class=${i} values="${Products[i].Category_id}">
                        <option value="">Select</option>
                        ${selectmap}
                    </select>
                    <div style="color: red; display: none;" class=${'editpcateerror'+i}></div>
                `

                for(var q = 0; q<document.getElementById('pcate').options.length; q++) {
                    if(parseInt(document.getElementById('pcate').options[q].value) === Products[i].Category_id) {
        ...
        if(product_len > 0) {
            return(
                product.map( function(p, i) { 
                    return ([<tr key={i} >
                        <td className="accordion-toggle collapsed" id={"accordion"+i} data-toggle="collapse" data-parent={"#accordion"+i} href={"#collapse"+i}>
                            <IoAddCircle style={{display: 'inline', cursor: 'pointer'}} className="plus icon_ind" onClick={() => {
                                document.getElementsByClassName('plus')[i].style.display = 'none'
                                document.getElementsByClassName('cl')[i].style.display = 'inherit'
                            }}
                            />
                            <IoRemoveCircleSharp style={{display: 'none', cursor: 'pointer'}} className="cl icon_ind" onClick={() => {
                                document.getElementsByClassName('plus')[i].style.display = 'inherit'
                                document.getElementsByClassName('cl')[i].style.display = 'none'
                                }} />
                        </td>
                        {/* <td></td> */}
                        <td><img src={JSON.parse(p.Image)[0] !== undefined ? `https://dtodo-indumentaria-server.herokuapp.com/${JSON.parse(p.Image)[0][0]}`: null} alt="" style={{width: '50px'}} /></td>
                        <td className={p.Name}>{p.Name}</td>
                        <td>{p.Category.Name}</td>
                        <td>{p.Description} </td>
                        <td className="edit">
                            <IoCloseCircle style={{display: 'inline'}} className="close_icon_ind" onClick={() => remove(p.Product_id)} />
                            <AiFillEdit style={{display: 'inline'}} className="edit_icon_ind" onClick={() => edit(i)} />
                        </td>
                    </tr>,
                    
                    <tr key={'inner'+i} className="hide-table-padding">
                        <td></td>
                        <td colSpan='5' className="table_inner">
                            {
    ...
    return (
        <div className="product_table">
            <div id="product_scroll" style={{overflowY: 'scroll', height: '700px'}} onScroll={handleScroll}>
                <Table striped hover>
                    <thead>
                        <tr className="head">
                            <th>#</th>
